...
#define LOAD_JPG_TYPE 1
#define LOAD_PNG_TYPE 2
...
unsigned loadPicAndBind(const char* path,unsigned _type,unsigned gl) {
	...
	if (data) {
		if (_type == LOAD_JPG_TYPE)
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
		else if (_type == LOAD_PNG_TYPE)
...
int main() {
	...
	unsigned int texture1 = loadPicAndBind("F:\\Code\\Graphic_Course\\HW1\\code\\container.jpg",LOAD_JPG_TYPE,GL_CLAMP_TO_EDGE);
	unsigned int texture2 = loadPicAndBind("F:\\Code\\Graphic_Course\\HW1\\code\\awesomeface.png",LOAD_PNG_TYPE,GL_MIRRORED_REPEAT);
	
	glUseProgram(shaderProgram);
	glUniform1i(glGetUniformLocation(shaderProgram, "texture1"), 0);
	glUniform1i(glGetUniformLocation(shaderProgram, "texture2"), GL_TEXTURE1 - GL_TEXTURE0);
