...
class KNN:
    def __init__(self, train_path, valid_path, test_path, K, distance_rule, vote_rule):
        '''
        ...
        self.VoteWithDistance = vote_rule
        ...
        if WordDict is None or WordList is None:
            ...
            for Target in data:
                for word in Target:
                    if word in WordDict:
                        NewDict[word] += 1
            IDF_MAT = [0 for i in range(len(NewDict))]
            TF_MAT = [[0 for i in range(len(NewDict))] for i in range(len(data))]
            for i in range(len(data)):
                Target = data[i]
    ...
    def run(self):
        ...
        self.load_test()
        self.y_Predict = [0 for i in range(len(self.y_Valid))]
        for i in range(len(self.x_Valid)):
            VoteDict = {}
            KNeighbour = self.get_k_nearest(self.x_Valid[i])
            for neighbour in KNeighbour:
                Vote = self.y_Train[neighbour[1]]
                if Vote not in VoteDict:
                    VoteDict.update({Vote: 1 if not self.VoteWithDistance else 1 / neighbour[0]})
                else:
                    VoteDict[Vote] += 1 if not self.VoteWithDistance else 1 / neighbour[0]
            Max, Result = 0, 'Error'
